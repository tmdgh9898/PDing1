...
def download_quaternary(info: dict) -> bool:
    ...
    # Download video-only for each resolution
    for res in VIDEO_RESOLUTIONS:
        video_name = f"{base}_video"
        ...
        try:
            ...
            continue
        # For downloaded video-only, download best audio then merge
        for aq in AUDIO_QUALITIES:
            audio_name = f"{base}_audio"
            ...
            try:
                ...
                continue
            # Merge
            merged_path = os.path.join(TEMP_DIR, f"{base}_merged.mp4")
            cmd = [
            ...
            try:
                ...
                move_to_android(merged_path, base)
                return True
            except:
                continue
    return False
